 ...
 */
/**
 * A player's page, whoever is looking at it.
 *
 * The player reaches it through his own link; a coach reaches the same thing
 * from his profile, because "what does he actually see?" is a question worth
 * being able to answer without borrowing his phone. `preview` only changes the
 * chrome — the page itself is his.
 */
export async function PlayerWork({ player, preview = false }: { player: Player; preview?: boolean }) {
  ...
  return (
    <>
      {preview && (
        <div className="text-center text-xs font-bold py-2 print:hidden"
          style={{ background: '#FDF6E3', color: '#7A5B12' }}>
          You are looking at {player.name}&rsquo;s page as he sees it. Nothing here is editable.
        </div>
      )}
