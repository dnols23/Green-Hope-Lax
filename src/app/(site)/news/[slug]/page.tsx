...
export default async function NewsPostPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  return (
    <article className="max-w-2xl mx-auto px-4 py-10">
      ...
      <div className="text-xs font-semibold text-gray-400 mt-4">{formatDate(post.published_at, { year: 'numeric' })}</div>
      <h1 className="page-title mt-1">{post.title}</h1>
      {post.image_url && (
        // eslint-disable-next-line @next/next/no-img-element
        <img src={post.image_url} alt="" className="w-full rounded-xl mt-6" />
      )}
